...
class NumberGuessingGameGUI:
    ...
    def __init__(self):
        ...
        # Label 1st range
        self.label1 = Label(root, font=self.font, text="1st Range: ", foreground = 'blue')
        ...
        self.entry1 = Entry(root, textvariable=self.num1, foreground='black')
        self.entry1.grid(row=0, column=1, padx=5, pady=5)

        # Label 2nd range
        self.label2 = Label(root, font=self.font, text="2nd range:", foreground = 'blue')
        self.label2.grid(row=1, column=0, sticky=W, padx=5, pady=5)

        # Entry Box For Storing 2st range Value
        self.entry2 = Entry(root, textvariable=self.num2, foreground='black')
        self.entry2.grid(row=1, column=1, padx=5, pady=5)

        # 
        self.btn = Button(root, font=self.font, text="Generate Random Number", command=self.randmbtn ,background='pink', foreground='blue')
        ...
        self.entry3 = Entry(root, textvariable=self.guessnum, font=self.font, foreground='black')
        self.entry3.grid(row=6, columnspan=2, pady=10, padx=5)

        # Button to check that value matches to random number or not
        self.btn = Button(root, font=self.font, text="Check", command=self.game,background='yellow', foreground='black')
        ...
        # Button to check that value matches to random number or not
        self.btn = Button(root, font=self.font, text="Exit", command=self.exitbtn ,background='yellow', foreground='black')
